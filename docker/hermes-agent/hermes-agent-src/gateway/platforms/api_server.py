...
if AIOHTTP_AVAILABLE:
    ...
    async def cors_middleware(request, handler):
        ...
        request_origin = f"{request.scheme}://{request.host}" if origin else ""
        same_origin = bool(origin and origin == request_origin)
        cors_headers = None
        if adapter is not None:
            if not same_origin and not adapter._origin_allowed(origin):
                return web.Response(status=403)
            cors_headers = None if same_origin else adapter._cors_headers_for_origin(origin)
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _handle_root(self, request: "web.Request") -> "web.Response":
        """GET / — serve the local admin console shell."""
        return await self._admin_service.handle_root(request)

    def _build_test_app(self) -> "web.Application":
        """Build an aiohttp app with the API server routes for unit tests."""
        ...
        app.router.add_post("/api/admin/test-connection", self._admin_service.handle_test_connection)
        app.router.add_post("/api/admin/apply", self._admin_service.handle_apply)
        app.router.add_get("/api/admin/status", self._admin_service.handle_status)
        app.router.add_post("/api/admin/restore", self._admin_service.handle_restore)
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app.router.add_get("/api/admin/profiles", self._admin_service.handle_list_profiles)
            self._app.router.add_post("/api/admin/profiles", self._admin_service.handle_create_profile)
            self._app.router.add_patch("/api/admin/profiles/{profile_id}", self._admin_service.handle_update_profile)
            self._app.router.add_delete("/api/admin/profiles/{profile_id}", self._admin_service.handle_delete_profile)
            self._app.router.add_post("/api/admin/profiles/{profile_id}/test", self._admin_service.handle_test_profile)
            self._app.router.add_post("/api/admin/profiles/{profile_id}/default", self._admin_service.handle_set_default_profile)
            self._app.router.add_post("/api/admin/profiles/{profile_id}/activate", self._admin_service.handle_activate_profile)
